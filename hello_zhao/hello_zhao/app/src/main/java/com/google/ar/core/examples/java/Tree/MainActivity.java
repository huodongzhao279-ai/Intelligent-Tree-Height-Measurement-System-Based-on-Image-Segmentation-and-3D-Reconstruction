...
public class MainActivity extends AppCompatActivity implements SampleRender.Renderer {
  ...
  private static final String SEARCHING_PLANE_MESSAGE = "特征点匹配中...";
...
//                saveBitmapFile(bitmap,MainActivity.this);
                ...
                if (cameraImage != null) {
                  ...
                  Bitmap cameraImageBitmap = YUV_420_888_toRGB(cameraImage, cameraImage.getWidth(), cameraImage.getHeight());
                   rootPath = createRootPath(MainActivity.this);
                  Log.d("Path", rootPath);ImageName = saveBitmapFile(cameraImageBitmap, rootPath);
                  if (ImageName != null) {
                    isSave[0] = 1;
                  } else isSave[0] = 0;
                  //保存深度图
...
//                  isSave[6]=saveBitmapFileWithName(confidenceBitmap,rootPath,ImageName+"_confidencedata",MainActivity.this);
                  ...
                  Toast.makeText(MainActivity.this, "尚未生成深度图", Toast.LENGTH_SHORT).show();
                }
              }
            }
    );
...
//        获取相机权限
    ...
    try {
      ...
      messageSnackbarHelper.showError(this, "无法调用相机，请重启app");
      session = null;
      return;
    }
...
//      如果会话在GLSurfaceView之前暂停，GLSurfaceView仍可能调用Session.update（）并产生SessionPausedException。
  ...
  public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] results) {
    ...
    if (!CameraPermissionHelper.hasCameraPermission(this)) {
      Toast.makeText(this, "需要获取相机权限", Toast.LENGTH_LONG)
              .show();
...
//    从ARSession中获取当前帧。当配置设置为UpdateMode时。BLOCKING(默认值)，这将限制渲染到相机帧率。
    try {
      ...
      Log.e(TAG, "onDrawFrame时相机不可用", e);
      messageSnackbarHelper.showError(this, "相机不可用，请重启");
      return;
    }
...
//      messageSnackbarHelper.hide(this);
      ...
      Handler handler = new Handler(Looper.getMainLooper()); // 如果这个handler是在UI线程中创建的
      handler.postDelayed(new Runnable() {  // 开启的runnable也会在这个handler所依附线程中运行，即主线程
        @Override
        public void run() {
          // 可更新UI或做其他事情
